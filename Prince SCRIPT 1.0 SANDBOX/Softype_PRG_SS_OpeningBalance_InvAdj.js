...
function scheduled(type) {
	...
	var processFolder = 248;
	var ProcessedFolder = 2;
	...
	if(totalFiles)
	{
		...
		{
			nlapiLogExecution('Audit', 'Usage:'+t, nlapiGetContext().getRemainingUsage());

			var fileID = totalFiles[t].getId();
			var fileObj = nlapiLoadFile(fileID);
			processTransaction(fileObj);
...
function processTransaction(fileObj){
	...
	if(recId){
		...
		var recData = nlapiLookupField('customrecord_prg_inventory_adjustment',recId,['custrecord_items_array','custrecord_valid_items','custrecord_invalid_items']);
	...
	for(var r = 0; r < itemLength; r++){
		...
		if(usage < 500){
			...
			var status = nlapiScheduleScript('customscript_create_inventory_adjustment','customdeploy_create_inventory_adjustm12',params);
...
//	      var searchUom = nlapiSearchRecord('unitstype',null,uom_filters,uom_columns);
		...
		for(var k = 0; k < rows.length; k++){
		    ...
		    if(documentnumber){
				    ...
				    JsonRecords.push({documentnumber:documentnumber,date:date,location:location,item:item,adjustqty:adjustqty,salesuom:salesuom,salesamount:salesamount,subsidiary:subsidiary});
		    }
		...
		if(recId){
			...
			createRecordObj.setFieldValue('custrecord_unit_cost','T');
