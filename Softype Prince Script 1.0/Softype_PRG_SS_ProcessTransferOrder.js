...
function scheduled(type) {
	...
	var processFolder = 267
	var ProcessedFolder = 268;
	...
	if(totalFiles)
	{
		...
		for(var t = 0; t < totalFiles.length; t++)
		{
			...
			if(usage <= 500){
				...
				var status = nlapiScheduleScript('customscript_ss_create_transferorder','customdeploy_ss_create_transferorder');
...
function processTransaction(fileObj,fileName,fileID){
	...
	for(var r = 0; r < itemLength; r++){
		...
		if(usage1 <= 500){
			...
			var status = nlapiScheduleScript('customscript_ss_create_transferorder','customdeploy_ss_create_transferorder',params);
