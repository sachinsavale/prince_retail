	...
	//	nlapiLogExecution('Debug','locationId',locationId);
		...
		else if(action == 'createInvoice')
		{
			...
			if(search != '' && search != null)
			{
				...
				createInvoiceObj.setFieldValue('customform',100)
				...
				nlapiLogExecution('debug','Line Items Length',lineitems.length);
				  ...
				  nlapiLogExecution('debug','Done Setting Line Item',inventoryAdjIds.length);
				  nlapiLogExecution('debug','Done Setting Line Item',inventoryAdjIds);
				  createInvoiceObj.setFieldValue('custbody_relatedvendorcr_ck',inventoryAdjIds);
