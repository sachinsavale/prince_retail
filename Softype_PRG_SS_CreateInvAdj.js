...
function scheduled(type) {
	...
	if(totalFiles)
	{
		...
		for(var t = 0; t < totalFiles.length; t++)
		{
		 ...
		 var flag = processTransaction(fileObj,fileName,fileID);
...
function processTransaction(fileObj,fileName,fileID){
	...
	for(var r = 0; r < itemLength; r++){
		...
		if(usage1 <= 500){
			...
			var status = nlapiScheduleScript('customscript_create_inventory_adjustment','customdeploy_create_inventory_adjustment',params);
